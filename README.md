...
可以看到還是存在一些漏檢，尤其是旁邊的女生那麼明顯地未戴口罩卻沒有被檢測出來，而且明明只有4個人卻檢測到5個人，所以我們又使用了另一張來測試

![image](mask1.jpg)

輸出結果如下圖所示。結果提示辨識到了1個戴口罩，2個未戴口罩。

![image](28.png)

識別後的結果存放在資料夾runs/detect/exp4中。打開該資料夾中的mask.jpg文件，識別結果如下圖所示。

![image](result1_yolov5s.jpg)

目前本文示範使用的是利用yolov5s進行的訓練，yolo5s是yolov5系列中最小的網路模型。如果想要提高檢測精度，一方面可以使用用更複雜的模型（如yolov5m、yolov5l、yolov5x等）進行訓練，另一方面可以提高樣本資料品質以及增加訓練集樣本數量（這裡訓練集只有149張圖片）。

所以如果想改善問題，我們針對目前的情況（在Google Colab上使用Tesla T4 GPU，且數據集規模看起来不算特別大），會最推薦你嘗試YOLOv5m，因為YOLOv5的模型從s到x，參數越來越多，雖然精度會提升，但訓練難度和硬體需求也呈指數級上升。
...
雖然還是可以看到一個漏檢，但已經可以準確判斷為4個人，並且跟YOLOv5s模型辨識出來的結果相比準確判斷有戴口罩和沒戴口罩的信心度都有增加，並且辨識錯誤的那個信心度也有降低。

而另一張的輸出結果如下圖所示。結果依然提示辨識到了個戴口罩，2個未戴口罩。

![image](29.png)

識別後的結果存放在資料夾runs/detect/exp5中。打開該資料夾中的mask.jpg文件，識別結果如下圖所示。

![image](result1_yolov5m.jpg)

從這張圖看到雖然便是結果依然是對的，但跟YOLOv5s模型辨識出來的結果相比判斷有戴口罩和沒戴口罩的信心度都有增加。

所以如果未來要進行訓練還是應該使用YOLOv5m模型，會比使用YOLOv5s模型來的準確。
